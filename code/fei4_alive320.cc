...
int main(int argc,char **argv) {
  unsigned int bits[21] = { 0x00000000, 0x00000000, 0x00000000,
                            ...
                            0x00000000, 0x00000000, 0x00000000 };
  if ( argc < 3 ) {
    cerr << "Usage: " << argv[0] << " <addr> <file>" << endl;
    exit(1);
  }
  TFile *fout = new TFile(argv[2],"RECREATE");
  fout->cd();
  TH2F *h_alive = new TH2F("alive",NULL,80,0.5,80.5,336,0.5,336.5);
...
//    for ( int icol=1; icol<=1; icol++ ) {
  for ( int irow=1; irow<=336; irow++ ) {
    for ( int icol=1; icol<=80; icol++ ) {
      fei4->ClearBuffer();
      fei4->Buffer()->Write(FEI4_CMD(ichan),0xaa070000);   // conf mode
      fei4->Buffer()->WaitClear(FEI4_CMD(ichan),0x80000000);
      fei4->WriteBuffer();
...
//   01bc : EOF
      ...
      fei4->ClearBuffer();
      fei4->Buffer()->Write(0xc80010c4,(irow<<16)|(icol<<25)|0x0c);
      fei4->Buffer()->Write(0xc8001000,0x40000000);
      fei4->Buffer()->Write(FEI4_CMD(ichan),0xaa380000);   // run mode
      fei4->Buffer()->WaitClear(FEI4_CMD(ichan),0x80000000);
      fei4->Buffer()->Write(0xc8000004,0x80660401);
      fei4->WriteBuffer();
      unsigned int cnt[16];
      fei4->ReadMatchCounters(ichan,cnt);
      h_alive->Fill(icol,irow,cnt[1]&0xffff);
    }
  }
  fout->Write();
  fout->Close();
