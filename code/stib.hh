...
namespace stib {
  ...
  const unsigned int STRIP_BCO_COUNTER_LOW = 0xc4000040;
  const unsigned int STRIP_BCO_COUNTER_HIGH = 0xc4000044;
  const unsigned int STRIP_STREAM_STATUS = 0xc400004c;
  ...
  const unsigned int GBE_STREAMPKT_CSR = 0xc10000a0;
...
//  const int BUFLEN = 1024;
  ...
  class Stib {
    ...
    unsigned char *_pbuf;
    unsigned int _iseq;
    unsigned char _txbuf[BUFLEN+256]; // we check for overflows AFTER the fact. Pad the buffer to avoid memory overwrites.
  ...
  public:
    ...
    void Clear();
    int Nop(unsigned int n=0);
    int Write(unsigned int,unsigned int);
    int SetBits(unsigned int,unsigned int);
    int ClearBits(unsigned int,unsigned int);
